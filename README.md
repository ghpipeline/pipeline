...
We use the following core metrics to evaluate performance:

- **Accuracy:** The percentage of all predictions the model got right.
- **Precision (for Class I):** When the model predicts a Class I recall, how often is it correct?
- **Recall (for Class I):** Of all true Class I recalls, how many did the model correctly catch?
- **ROC AUC:** Measures how well the model distinguishes between Class I and non-Class I recalls, regardless of threshold.

> Conceptually, **precision** answers “How much can we trust a positive prediction?” while **recall** asks “How many true positives are we successfully identifying?”

---

### What These Metrics Mean in Our Context

In the context of FDA drug recalls:

- **High recall** is critical — we want to **catch as many true Class I recalls as possible**, even if it means risking some false alarms (false positives).
- **Moderate precision** is acceptable — false positives are less dangerous than missing a truly hazardous recall.
- **High ROC AUC** indicates that our model is generally good at separating risky vs. non-risky recalls, even before applying a threshold.

> A strong recall score tells us our model could act as an early warning system for high-risk recalls — helping regulators or health systems focus their attention where it matters most.
...
### Real-World Impact
This pipeline represents more than just a technical project, it’s a practical step toward improving drug safety. By predicting which recalls are likely to be the most serious, it empowers agencies, healthcare providers, and supply chain managers to:
